...
- The `SpatialDeploymentManager` now requires a `runtimeVersion` parameter.
- Removed the `SpatialdDeploymentManager` class.

### Added

- Added a set of extension methods for the `ILogDispatcher` for the common cases of `Info`/`Warn`/`Error`.
...
- The `WorldCommandSender` correctly has its callbacks removed when the containing Monobehaviour is disabled.
