...
namespace Improbable.Gdk.Core
{
    ...
    public class WorldCommandsSendSystem : ComponentSystem
    {
        ...
        private readonly EntityArchetypeQuery worldCommandSendersQuery = new EntityArchetypeQuery
        {
            All = new[]
            {
                ComponentType.ReadOnly<WorldCommands.CreateEntity.CommandSender>(),
                ComponentType.ReadOnly<WorldCommands.DeleteEntity.CommandSender>(),
                ComponentType.ReadOnly<WorldCommands.ReserveEntityIds.CommandSender>(),
                ComponentType.ReadOnly<WorldCommands.EntityQuery.CommandSender>(),
            },
        ...
        protected override void OnUpdate()
        {
            ...
            foreach (var chunk in chunkArray)
            {
                ...
                {
                    var requestsToChange = deleteEntitySenders[i].RequestsToSend;
                    var entity = entityArray[i];
                    foreach (var req in requestsToChange)
                    {
                ...
                {
                    var requestsToChange = entityQuerySenders[i].RequestsToSend;
                    var entity = entityArray[i];
                    foreach (var req in requestsToChange)
                    {
