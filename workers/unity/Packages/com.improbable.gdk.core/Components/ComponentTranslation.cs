...
namespace Improbable.Gdk.Core.Components
{
    public abstract class ComponentTranslation : IDisposable
    {
        ...
        protected ComponentTranslation(Worker worker)
        {
            Worker = worker;
            EntityManager = worker.World.GetOrCreateManager<EntityManager>();
            ...
            where T : Component
        {
            ...
            for (var i = 0; i < entityArray.Length; i++)
            {
                ...
                if (EntityManager.HasComponent<T>(entity))
                {
                    pool.PutComponent(EntityManager.GetComponentObject<T>(entity));
                    entityCommandBuffer.RemoveComponent<T>(entity);
                }
            }
        }
        ...
        public void HandleAuthorityChange<T>(long entityId, Authority authority, ComponentPool<AuthoritiesChanged<T>> pool)
        {
            ...
            var bufferedComponents = EntityManager.HasComponent<AuthoritiesChanged<T>>(entity)
                ? EntityManager.GetComponentObject<AuthoritiesChanged<T>>(entity)
                : pool.GetComponent();
            
            bufferedComponents.Buffer.Add(authority);
            EntityManager.SetComponentObject(entity, bufferedComponents);
        }
        ...
        {
            var messageBuffer = EntityManager.HasComponent<TMessagesReceived>(entity) ? EntityManager.GetComponentObject<TMessagesReceived>(entity) : pool.GetComponent();
            messageBuffer.Buffer.Add(message);
            EntityManager.SetComponentObject(entity, messageBuffer);
        }
