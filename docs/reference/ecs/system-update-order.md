...
Unity provides attributes to define the [update order of systems (Unity documentation)](https://github.com/Unity-Technologies/EntityComponentSystemSamples/blob/master/Documentation/reference/ecs_in_detail.md#system-update-order). These attributes are: `UpdateInGroup`, `UpdateBefore` and `UpdateAfter`.
...
* `SpatialOSSendGroup` - This group contains all systems related to sending data to SpatialOS .
  ...
  * `InternalSpatialOSCleanGroup` - This group is used by the `CleanReactiveComponentsSystem` to remove all [reactive](\{\{urlRoot\}\}/reference/ecs/reactive-components) and [temporary components](\{\{urlRoot\}\}/reference/ecs/temporary-components) at the end of each update loop. **(Internal use only)**
