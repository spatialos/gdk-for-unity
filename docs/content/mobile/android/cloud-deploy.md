...
1. The `MobileWorkerConnector` provides a `DevelopmentAuthToken` field. In the same drop-down window, add the authentication token you created to the `DevelopmentAuthToken` field.
