...
#### Several warnings when destroying a GameObject or ECS entity that represents a SpatialOS entity

**Cause**<br/>
The GDK is designed so that, rather than deleting GameObjects or ECS entities that represent SpatialOS entities. The GDK cleans them up automatically when the respective SpatialOS entity is removed from the worker.

**Fix**<br/>
Send a [DeleteEntity world command]({{urlRoot}}/content/gameobject/world-commands.md) to delete the entity on the SpatialOS side instead.
