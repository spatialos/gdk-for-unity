...
The FPS Starter Project uses the `AdvancedEntityPipeline` implementation of the [`IEntityGameObjectCreator` interface]({{urlRoot}}/api/game-object-creation/i-entity-game-object-creator) from the [GameObject Creation]({{urlRoot}}/content/modules/core-and-feature-module-overview#gameobject-creation-module) feature module to handle the instantiation of GameObjects to represent SpatialOS entities.
