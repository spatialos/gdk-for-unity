...
namespace Improbable.Tests
{
    public partial class DependencyTestChild
    {
        public class EcsViewManager : IEcsViewManager
        {
            ...
            {
                var entity = workerSystem.GetEntity(entityId);
                entityManager.RemoveComponent<global::Improbable.Tests.DependencyTestChild.HasAuthority>(entity);
            ...
            private void ApplyUpdate(in ComponentUpdateReceived<Update> update, ComponentDataFromEntity<Component> dataFromEntity)
            {
                ...
                if (update.Update.Child.HasValue)
                {
                    data.Child = update.Update.Child.Value;
                }
            ...
            private void SetAuthority(EntityId entityId, Authority authority)
            {
                switch (authority)
                {
                    ...
                    {
                        var entity = workerSystem.GetEntity(entityId);
                        entityManager.AddComponent<global::Improbable.Tests.DependencyTestChild.HasAuthority>(entity);
                        break;
                    }
