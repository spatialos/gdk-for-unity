...
namespace Improbable.TestSchema
{
    ...
    {
        public class ComponentUsingNestedTypeSameNameViewStorage : IViewStorage, IViewComponentStorage<Snapshot>, IViewComponentUpdater<Update>
        {
            ...
            public void ApplyUpdate(long entityId, in Update update)
            {
                ...
                {
                    data.NestedField = update.NestedField.Value;
                }

                if (update.Other0Field.HasValue)
                {
                    data.Other0Field = update.Other0Field.Value;
                }

                if (update.Other1Field.HasValue)
                {
                    data.Other1Field = update.Other1Field.Value;
                }
