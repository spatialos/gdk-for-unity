    ...
    public partial class BlittableComponent
    {
        internal class DispatcherHandler : ComponentDispatcherHandler
        {
            ...
            public override void OnComponentUpdate(ComponentUpdateOp op)
            {
                ...
                var eventsObject = op.Update.SchemaData.Value.GetEvents();
                {
                    ...
                    if (eventCount > 0)
                    {
                        ...
                        {
                            var e = global::Improbable.Gdk.Tests.BlittableTypes.FirstEventPayload.Serialization.Deserialize(eventsObject.IndexObject(1, i));
                            eventList.Add(e);
                        }
                    }
                }

                {
                    ...
                    if (eventCount > 0)
                    {
                        ...
                        for (uint i = 0; i < eventCount; i++)
                        {
                            var e = global::Improbable.Gdk.Tests.BlittableTypes.SecondEventPayload.Serialization.Deserialize(eventsObject.IndexObject(2, i));
                            eventList.Add(e);
                        }
                    }
                }
            ...
            private void OnFirstCommandRequest(CommandRequestOp op)
            {
                ...
                requests.Add(new Improbable.Gdk.Tests.BlittableTypes.BlittableComponent.FirstCommand.ReceivedRequest(op.RequestId,
                    op.CallerWorkerId,
            ...
            {
                if (!firstCommandStorage.CommandRequestsInFlight.TryGetValue(op.RequestId, out var requestBundle))
                {
                    throw new InvalidOperationException($"Could not find corresponding request for RequestId {op.RequestId} and command FirstCommand.");
                }

                var entity = requestBundle.Entity;
                firstCommandStorage.CommandRequestsInFlight.Remove(op.RequestId);
                ...
                responses.Add(new Improbable.Gdk.Tests.BlittableTypes.BlittableComponent.FirstCommand.ReceivedResponse(new EntityId(op.EntityId),
                    op.Message,
                    op.StatusCode,
                    response,
            ...
            private void OnSecondCommandRequest(CommandRequestOp op)
            {
                ...
                requests.Add(new Improbable.Gdk.Tests.BlittableTypes.BlittableComponent.SecondCommand.ReceivedRequest(op.RequestId,
                    op.CallerWorkerId,
                    op.CallerAttributeSet,
                    deserializedRequest));
            }
            ...
            {
                if (!secondCommandStorage.CommandRequestsInFlight.TryGetValue(op.RequestId, out var requestBundle))
                {
                    throw new InvalidOperationException($"Could not find corresponding request for RequestId {op.RequestId} and command SecondCommand.");
                }

                var entity = requestBundle.Entity;
                secondCommandStorage.CommandRequestsInFlight.Remove(op.RequestId);
                if (!entityManager.Exists(entity))
                {
                    LogDispatcher.HandleLog(LogType.Log, new LogEvent(EntityNotFound)
                        .WithField(LoggingUtils.LoggerName, LoggerName)
                ...
                responses.Add(new Improbable.Gdk.Tests.BlittableTypes.BlittableComponent.SecondCommand.ReceivedResponse(new EntityId(op.EntityId),
                    op.Message,
                    op.StatusCode,
                    response,
                    requestBundle.Request,
        ...
        internal class ComponentReplicator : ComponentReplicationHandler
        {
            ...
            public override void ExecuteReplication(ComponentGroup replicationGroup, ComponentSystemBase system, global::Improbable.WorkerCore.Connection connection)
            {
                Profiler.BeginSample("BlittableComponent");
                ...
                foreach (var chunk in chunkArray)
                {
                    ...
                    for (var i = 0; i < componentArray.Length; i++)
                    {
                        ...
                        {
                            var update = new global::Improbable.WorkerCore.SchemaComponentUpdate(1001);
                            ...
                            connection.SendComponentUpdate(entityIdArray[i].EntityId.Id, new global::Improbable.WorkerCore.ComponentUpdate(update));
            ...
            public override void SendCommands(ComponentGroup commandGroup, ComponentSystemBase system, global::Improbable.WorkerCore.Connection connection)
            {
                Profiler.BeginSample("BlittableComponent");
                    ...
                    foreach (var chunk in chunks)
                    {
                        ...
                        for (var i = 0; i < senders.Length; i++)
                        {
                            ...
                            if (requests.Count > 0)
                            {
                                ...
                                {
                                    var schemaCommandRequest = new global::Improbable.WorkerCore.SchemaCommandRequest(ComponentId, 1);
                                    ...
                                    var requestId = connection.SendCommandRequest(request.TargetEntityId.Id,
                                        new global::Improbable.WorkerCore.CommandRequest(schemaCommandRequest),
                                        request.TimeoutMillis,
                                        request.AllowShortCircuiting ? ShortCircuitParameters : null);

                                    firstCommandStorage.CommandRequestsInFlight[requestId] =
                            ...
                            if (responses.Count > 0)
                            {
                                foreach (var response in responses)
                                {
                                    ...
                                    var schemaCommandResponse = new global::Improbable.WorkerCore.SchemaCommandResponse(ComponentId, 1);
                                    ...
                                    connection.SendCommandResponse(requestId, new global::Improbable.WorkerCore.CommandResponse(schemaCommandResponse));
                                }
                    ...
                    foreach (var chunk in chunks)
                    {
                        ...
                        for (var i = 0; i < senders.Length; i++)
                        {
                            ...
                            if (requests.Count > 0)
                            {
                                foreach (var request in requests)
                                {
                                    var schemaCommandRequest = new global::Improbable.WorkerCore.SchemaCommandRequest(ComponentId, 2);
                                    ...
                                    secondCommandStorage.CommandRequestsInFlight[requestId] =
                            ...
                            if (responses.Count > 0)
                            {
                                foreach (var response in responses)
                                {
                                    ...
                                    var schemaCommandResponse = new global::Improbable.WorkerCore.SchemaCommandResponse(ComponentId, 2);
        ...
        internal class AcknowledgeAuthorityLossHandler : AbstractAcknowledgeAuthorityLossHandler
        {
            ...
            public override void AcknowledgeAuthorityLoss(ComponentGroup group, ComponentSystemBase system,
                Improbable.WorkerCore.Connection connection)
            {
