...
namespace Improbable.Gdk.Tests
{
    ...
    public class ExhaustiveOptionalReaderSubscriptionManager : SubscriptionManager<ExhaustiveOptionalReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<ExhaustiveOptionalReader>>> entityIdToReaderSubscriptions;
        ...
        public ExhaustiveOptionalReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(ExhaustiveOptional.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new ExhaustiveOptionalReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(ExhaustiveOptional.ComponentId, entityId =>
            {
        ...
        public override Subscription<ExhaustiveOptionalReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<ExhaustiveOptionalReader>>>();
            }

            var subscription = new Subscription<ExhaustiveOptionalReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<ExhaustiveOptionalReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<ExhaustiveOptional.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new ExhaustiveOptionalReader(world, entity, entityId));
            }
    ...
    [AutoRegisterSubscriptionManager]
    public class ExhaustiveOptionalWriterSubscriptionManager : SubscriptionManager<ExhaustiveOptionalWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<ExhaustiveOptionalWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public ExhaustiveOptionalWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(ExhaustiveOptional.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new ExhaustiveOptionalWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<ExhaustiveOptionalWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<ExhaustiveOptionalWriter>>>();
            }

            var subscription = new Subscription<ExhaustiveOptionalWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<ExhaustiveOptionalWriter>>();
                entityIdToWriterSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && componentUpdateSystem.HasComponent(ExhaustiveOptional.ComponentId, entityId)
                && componentUpdateSystem.GetAuthority(entityId, ExhaustiveOptional.ComponentId) != Authority.NotAuthoritative)
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new ExhaustiveOptionalWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
    ...
    public class ExhaustiveOptionalReader
    {
        public bool IsValid;
        ...
        public ExhaustiveOptional.Component Data
        {
            get
            {
                if (!IsValid)
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, ExhaustiveOptional.ComponentId, value);
        ...
        private Dictionary<Action<ExhaustiveOptional.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<ExhaustiveOptional.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<ExhaustiveOptional.Update>, ulong>();
                }
        ...
        internal ExhaustiveOptionalReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
